...
export const EmailTemplatesView = ({ school, onBack }) => {
  ...
  return (
    <div className="px-4 sm:px-6 lg:px-10 py-6 lg:py-8 max-w-5xl">
      {/* HEADER (in-content, matches other views) */}
      <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-3 mb-6">
        <div>
          ...
          >
            COACH OUTREACH <span className="text-blue-600">EMAIL TEMPLATES</span>
          </h1>
          <p className="text-slate-500 text-sm mt-1">Pre-filled, personalized, copy-ready — three phases of coach outreach.</p>
        </div>
        <button
          onClick={onBack}
      ...
      {/* STRATEGY BANNER */}
      <div className="rounded-2xl mb-6 p-6 border border-blue-200" style={{ background: 'linear-gradient(135deg, #eff6ff, #dbeafe)' }}>
        ...
        {tIds.map(id => {
          ...
          return (
            <button key={id} onClick={() => setActiveTab(id)}
              className={`flex-1 py-3 px-4 rounded-xl font-bold text-sm uppercase tracking-wide transition-all border ${isActive ? `${c.badge} text-white border-transparent shadow-lg` : 'bg-white text-slate-600 border-slate-200 hover:border-slate-300'}`}>
        ...
        <div className="p-6 space-y-5">
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <label className="text-[11px] font-bold text-slate-500 uppercase tracking-widest">Subject Line</label>
              <button onClick={() => copy(getSubject(activeTab), `subj-${activeTab}`)}
                className="flex items-center gap-1.5 text-[11px] font-bold text-slate-600 hover:text-blue-600 px-2 py-1 rounded-lg hover:bg-blue-50 transition-colors">
          ...
          <div className="flex flex-wrap gap-3 pt-2 border-t border-slate-100">
            ...
            <button onClick={() => { setEditedSubjects(p => { const n={...p}; delete n[activeTab]; return n; }); setEditedBodies(p => { const n={...p}; delete n[activeTab]; return n; }); }}
              className="flex items-center gap-2 px-4 py-3 rounded-xl font-bold text-sm text-slate-600 hover:text-slate-800 hover:bg-slate-100 transition-all">
              Reset to Default
            </button>
          </div>
        </div>
          ...
          {tIds.map(id => {
            ...
            return (
              <div key={id} className={`rounded-2xl border p-5 ${c.light}`}>
                <span className={`text-[11px] font-bold px-2 py-0.5 rounded-full text-white ${c.badge} inline-block mb-2`}>{t.phase}</span>
