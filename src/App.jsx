...
export default function App() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(saveSchools, 500); // Debounce saves
  ...
  const [driveAuth, setDriveAuth] = useState({ token: null, sheetId: null });
  const [lastSyncStatus, setLastSyncStatus] = useState({ school: null, status: null, message: null });
  ...
  const handleAddSchool = async () => {
    ...
    try {
      ...
      if (parsed.isVolleyballSchool === false) {
        ...
        const schoolData = { ...parsed, section: parsed.section || "discovery" };
        setExtraSchools(prev => [schoolData, ...prev]);
        setNewSchoolName('');
        // Google Drive sync happens automatically via useEffect
      }
            ...
            {isLoadingDrive ? (
              <>
                <div className="font-bold text-emerald-900 text-sm flex items-center gap-2">
                  <Loader2 className="w-4 h-4 animate-spin" /> Loading schools from Google Drive...
                </div>
                <div className="text-xs text-emerald-700 mt-0.5">Syncing your saved programs</div>
              </>
            ) : (
              <>
                <div className="font-bold text-emerald-900 text-sm flex items-center gap-2">
                  <Check className="w-4 h-4" /> Google Drive Sync Active
                </div>
                <div className="text-xs text-emerald-700 mt-0.5">All added schools automatically sync to Google Drive • {extraSchools.length} schools backed up</div>
              </>
            )}
          </div>
        </div>
